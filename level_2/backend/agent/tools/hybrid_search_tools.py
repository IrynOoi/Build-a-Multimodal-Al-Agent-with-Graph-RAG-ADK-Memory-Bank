...
async def semantic_search(query: str, limit: int = 10) -> str:
    ...
    Use this when you specifically want to find things by MEANING, not just matching keywords. Great for:
    - Finding conceptually similar items
    - Handling vague or abstract queries
    - When exact terms are unknown
    
    Example:
        query = "healing abilities"
        This might return "first aid", "surgery", "herbalism" even though no keywords match exactly.
    
    Args:
        query (str): What you're looking for (describe the concept)
        limit (int): Maximum results to return
    
    Returns:
        str: Semantically similar results ranked by relevance
    """
    try:
        service = _get_service()
