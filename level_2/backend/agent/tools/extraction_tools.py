...
# ---------------------------------------------------------------------------
# Status normalization — SurvivorHasNeed.status column limit is 20 chars
# ---------------------------------------------------------------------------
...
def _normalize_status(raw_status: str, max_len: int = 20) -> str:
    """Normalize a status string to fit within the Spanner column size limit."""
    if not raw_status:
        return "unknown"
    cleaned = raw_status.lower().strip()
    if cleaned in _VALID_STATUSES:
        return _VALID_STATUSES[cleaned]
    # Partial match
    for key, value in _VALID_STATUSES.items():
        if key in cleaned or cleaned in key:
            return value
    # Hard truncate as last resort
    logger.warning(f"Unrecognized status '{raw_status}' truncated to '{cleaned[:max_len]}'")
    return cleaned[:max_len]


def _sanitize_result_dict(result_dict: Dict[str, Any]) -> Dict[str, Any]:
    """Sanitize extraction result dict before saving to fix column size violations."""
    ...
    for entity in result_dict.get("entities", []):
        ...
        if "status" in entity:
            entity["status"] = _normalize_status(entity["status"])
        # Nested properties dict
        props = entity.get("properties", {})
        if isinstance(props, dict) and "status" in props:
            props["status"] = _normalize_status(props["status"])

    for rel in result_dict.get("relationships", []):
        if not isinstance(rel, dict):
            continue
        if "status" in rel:
            rel["status"] = _normalize_status(rel["status"])
...
def save_to_spanner(extraction_result: Any, survivor_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        # If result_obj is a dict (from to_dict()), sanitize then reconstruct it
        if isinstance(result_obj, dict):
            result_obj = _sanitize_result_dict(result_obj)  # <-- normalize status fields
